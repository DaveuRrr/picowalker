...
static lv_color_t buffer0[SH8601A_WIDTH * SH8601A_HEIGHT / LVGL_BUFFER_DIVISOR];  // ~33KB
static lv_color_t buffer1[SH8601A_WIDTH * SH8601A_HEIGHT / LVGL_BUFFER_DIVISOR];  // ~33KB
#if CANVAS_SCALE >= 3
// For CANVAS_SCALE >= 3: Use full-size image buffer without zoom
static lv_color_t image_buffer[CANVAS_WIDTH * CANVAS_HEIGHT]; // 288x192 = 110KB (WARNING: HIGH RAM USAGE)
...
static lv_obj_t *tile_picowalker; // only for clearing the edges...
...
static void display_flush_callback(lv_disp_drv_t *display, const lv_area_t *area, lv_color_t *color)
{
    ...
    // Set display window
    SH8601A_Set_Windows(area->x1, area->y1, area->x2 + 1, area->y2 + 1);

    // Start pixel write using QSPI
    QSPI_Select(qspi);
    QSPI_Pixel_Write(qspi, 0x2c);

    // Switch to 4-wire mode for data transfer
    QSPI_4Wrie_Mode(&qspi);

    // Configure and start DMA transfer
    channel_config_set_dreq(&SH8601A_DMA_CONFIG, pio_get_dreq(qspi.pio, qspi.sm_4wire, true));
    dma_channel_configure(SH8601A_DMA_TX,
                          &SH8601A_DMA_CONFIG,
                          &qspi.pio->txf[qspi.sm_4wire],  // Destination: PIO TX FIFO
                          (uint8_t *)color,                // Source: color buffer
                          pixel_count * 2,                 // Transfer size in bytes
...
static void touch_irq_callback(uint gpio, uint32_t events)
{
    ...
    {
        FT3168_Get_Point(SCREEN_ROTATION, SH8601A.WIDTH, SH8601A.HEIGHT);
        touch_x = FT3168.x_point;
        touch_y = FT3168.y_point;
...
{
    if (dma_channel_get_irq0_status(SH8601A_DMA_TX))
    {
        dma_channel_acknowledge_irq0(SH8601A_DMA_TX);

        // Deselect QSPI after DMA transfer completes
        QSPI_Deselect(qspi);
...
void pw_screen_init()
{
    ...
    // Initialize WaveShare 1.8" AMOLED - Screen
    ...
    qspi.sm_1wire = 1;
    qspi.pin_cs = CS_PIN;
    qspi.pin_sclk = SCLK_PIN;
    qspi.pin_dio0 = DIO0_PIN;
    qspi.pin_dio1 = DIO1_PIN;
    qspi.pin_dio2 = DIO2_PIN;
    qspi.pin_dio3 = DIO3_PIN;
    qspi.pin_pwr_en = PWR_EN_PIN;
    qspi.pin_rst = RST_PIN;

    QSPI_GPIO_Init(qspi);
    QSPI_PIO_Init(qspi);
    QSPI_1Wrie_Mode(&qspi);

    SH8601A_Init(SCREEN_ROTATION);
    SH8601A_Set_Brightness(50);  // Start at 50% brightness
    SH8601A_Clear(BLACK);
...
#if TOUCH
    // Initialize Touch Screen - FT3168
    ...
    dma_channel_set_irq0_enabled(SH8601A_DMA_TX, true);
    ...
    tile_picowalker = lv_tileview_add_tile(tile_view, 0, 0, LV_DIR_BOTTOM);

    // Set tile background color to match canvas/image background
    lv_obj_set_style_bg_color(tile_picowalker, palettes[pw_color_mode][PW_SCREEN_WHITE], 0);
    ...
    // Note: LVGL timer (5ms tick) is started in main.c

    printf("[SH8601A] LVGL initialized - %dx%d display with QSPI PIO\n", DISP_HOR_RES, DISP_VER_RES);
}
...
void pw_screen_clear()
{
    ...
    lv_obj_invalidate(image_obj);
    lv_obj_set_style_bg_color(tile_picowalker, palettes[pw_color_mode][PW_SCREEN_WHITE], 0);
    lv_obj_invalidate(tile_picowalker);
...
/********************************************************************************
 * @brief           Changes brightness of the screen
 * @param level     Brightness level (0-PW_SCREEN_MAX_BRIGHTNESS)
********************************************************************************/
void pw_screen_set_brightness(uint8_t level)
{
    if(level > PW_SCREEN_MAX_BRIGHTNESS) return;

    uint8_t brightness_range = SH8601A_MAX_BRIGHTNESS - SH8601A_MIN_BRIGHTNESS + 1;
    uint8_t level_range = PW_SCREEN_MAX_BRIGHTNESS + 1;
    float step = (float)brightness_range / (float)level_range;
    uint8_t screen_setting = SH8601A_MIN_BRIGHTNESS + (uint8_t)((float)level * step);
    SH8601A_Set_Brightness(screen_setting);
