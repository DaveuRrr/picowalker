 ...
 * @param color     Color from the pw_screen_color_t (Greyscale)
 * @return lv_color_t
********************************************************************************/
...
void pw_screen_set_brightness(uint8_t level)
{
    ...
    uint8_t brightness_range = GC9A01A_MAX_BRIGHTNESS - GC9A01A_MIN_BRIGHTNESS + 1;
    uint8_t level_range = PW_SCREEN_MAX_BRIGHTNESS + 1;
    float step = (float)brightness_range / (float)level_range;
    uint8_t screen_setting = GC9A01A_MIN_BRIGHTNESS + (uint8_t)((float)level * step);
    GC9A01A_Set_PWM(screen_setting);
